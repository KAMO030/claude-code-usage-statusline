...
| `5h剩93%↻15:50 7d剩66%↻06-11 04:00` | **套餐额度** — 5 小时窗 + 7 天窗剩余,各带重置时刻(`↻`),临近上限变黄/红 | `GET /api/oauth/usage` |
...
| `RESET_STYLE` | `"clock"` | 5h/7d 重置显示:`clock` 重置时刻(如 `↻15:50`,跨天自动带日期 `↻06-11 04:00`)/ `countdown` 倒计时(`↻3h`)/ `both`(`↻15:50(3h)`)/ `off` 不显示 |

### 🌐 语言

显示语言**默认跟随系统**(读 `LC_ALL` → `LC_MESSAGES` → `LANG`),无需配置。支持 10 种:

| 取值 | 语言 | 示例(剩余段) |
|---|---|---|
| `zh` | 简体中文 | `(剩81%) · 5h剩93%` |
| `zh-TW` | 繁體中文 | `(剩81%) · 5h剩93%` |
| `en` | English | `(81% left) · 5h 93% left` |
| `ja` | 日本語 | `(残81%) · 5h残93%` |
...
> 系统语言无法识别时回退到 `en`;繁简通过 `zh_TW`/`zh_HK`/`Hant` 等区分。只有标签文字(剩余/重置)本地化,数字与模型名保持原样。
